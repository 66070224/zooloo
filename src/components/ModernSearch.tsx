...
function ModernSearch() {
    ...
    const [selectedAnimal, setSelectedAnimal] = useState(oceanData[0]);

    const handleSearch = (e: any) => {
        e.preventDefault();
    };

    const handleOpenPopup = (animal: any) => {
    ...
    const filteredData = oceanData.filter((item) => {
        ...
        const nicknameMatch = item.nickname.toLowerCase().includes(searchTerm.toLowerCase());
        return searchTerm === '' ? true : (nameMatch || nicknameMatch);
    });

    return (
        <div className="flex flex-col items-center w-full py-10 min-h-screen">
            <form onSubmit={handleSearch} className="mb-16 w-full max-w-xl px-4 sticky top-5 z-40">
                        ...
                        >
                            ...
                            <div className="p-6 text-center">
                                ...
                                <p className="text-blue-300/60 text-sm italic font-medium">
                                    {animal.nickname !== "None" ? animal.nickname : "Ocean Dweller"}
                                </p>
                            </div>
                        </motion.div>
                    ))
